    ...
    <PageCustomLayout>
      <Suspense fallback={<Loading />}>
        <Image
          ...
          width={500}
          height={300}
          className="w-[20vw] md:w-[35vw] h-auto mb-4 object-cover rounded-lg absolute md:bottom-[calc(100%-30vw)]
        ...
        <div className="grid grid-cols-2 md:grid-cols-1 gap-6">
          <div>
            <div className="flex flex-wrap gap-2 items-center w-full md:w-1/2 mb-2">
              {recipe.type && (
                <div className="flex items-center space-x-2 text-lg border-2 px-2 py-1 rounded-xl border-purple-500 rounded-xl bg-purple-50">
                  <span>{recipe.type}</span>
                </div>
              )}
              {recipe.difficulty && (
                <div className="flex items-center space-x-2 text-lg border-2 px-2 py-1 rounded-xl border-red-500 rounded-xl bg-red-50">
                  <Flame className="w-6 h-6 text-red-500" />
                  <span>Difficulté: {recipe.difficulty}</span>
                </div>
              )}
            </div>
            <div className="flex gap-2 flex-wrap items-center w-full md:w-1/2 mb-6">
              {recipe.category && (
                <div className="flex items-center space-x-2 text-lg border-2 px-2 py-1 rounded-xl border-cyan-500 rounded-xl bg-cyan-50">
                  <span>{recipe.category}</span>
                </div>
              )}
              {recipe.license && (
                <div className="flex items-center space-x-2 text-lg border-2 px-2 py-1 rounded-xl border-custom rounded-xl bg-custom-background">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card className="h-fit border-2 border-black">
            <CardHeader>
              <CardTitle className="text-xl text-custom">
                Instructions
              </CardTitle>
            </CardHeader>
            <CardContent>
